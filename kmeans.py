...
class KMeans(object):
    ...
    def initialize_centroids(self, data):
        ...
        for i in range(1, self.k):
            ...
            for j in range(len(self.centroids)):
                   distances = distances.append(self.get_euclidean_distance(sliced_data, self.centroids.iloc[j]), ignore_index = True)
            ...
            minVals = distances.min(axis = 0)
            index = minVals.idxmax()
    ...
    def get_euclidean_distance(self, point1, point2):
        ...
        if (isinstance(points, pd.DataFrame)):
            dist = np.sqrt(points.sum(axis = 1))
        else:
            dist = np.sqrt(points.sum())
        return dist
    ...
    def train(self, data, iters):
        ...
        while i < iters and not flag_groups and not flag_centroids:
            ...
            if (groups.equals(centroids)):
                flag_groups = True

            # TODO: Check if there are changes with the values of the centroids
            centroidvals = centroids.iloc[:, self.start_var:self.end_var]
            
            if(self.centroids.equals(centroidvals)):
                flag_centroids = True
