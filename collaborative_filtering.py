...
class CollaborativeFiltering(object):
    ...
    def get_cosine_similarity(self, vector1, vector2):
        ...
        if (isinstance(vector1, pd.DataFrame) and  isinstance(vector2, pd.Series)):
            return (vector1*vector2).sum(axis=1)/(np.sqrt((vector1*vector1).sum(axis=1))*np.sqrt((vector2*vector2).sum()))
        elif (isinstance(vector1, pd.Series) and  isinstance(vector2, pd.DataFrame)):
            return (vector1*vector2).sum(axis=1)/(np.sqrt((vector1*vector1).sum())*np.sqrt((vector2*vector2).sum(axis=1)))
        else:
            return (vector1*vector2).sum()/(np.sqrt((vector1*vector1).sum())*np.sqrt((vector2*vector2).sum()))
    ...
    def get_k_similar(self, data, vector):
        ...
        cosinesim = self.get_cosine_similarity(normvector, normdata)
        ...
        indices = []
        values = cosinesim.nlargest(self.k)
        for k in range(len(cosinesim)):
            for y in range(len(values)):
                if cosinesim[k] == values[y]:
                    indices.append(k)
    ...
    def get_rating(self, data, index, column):
        ...
        consinesim = self.get_k_similar(new_data, vector)
        consinesim = consinesim[1]
        # TODO: Compute for the rating using the similarity values and the raw
        # ratings for the k similar items.
        y = data.iloc[:, column]
        rating = y.loc[consinesim.index]
        
        rating = (consinesim*rating).sum() / consinesim.sum()
        return rating
